...
## [v3.0.4] - 2026-02-11 - 恢復桌面自動關閉與強化 MCP 迴圈規則

### 🌟 版本亮點
此補丁恢復桌面模式提交回饋後的自動關閉行為，並強化工具定義規則，要求代理在各階段持續迴圈呼叫互動回饋。

### ✨ 改進功能
- 恢復桌面模式：回饋提交後立即關閉桌面視窗
- 更新 `interactive_feedback` 工具規則，強化「任意階段都需繼續再次呼叫」
- 同步修正文案，確保提交後提示與自動關閉行為一致

---

## [v3.0.3] - 2026-02-11 - 超時安全加固

### 🌟 版本亮點
此補丁聚焦於消除互動回饋流程中的 timeout 誤觸發中斷問題。

### ✨ 改進功能
- 從 `interactive_feedback` 工具參數中移除 `timeout`，呼叫方無法再傳入極小值
- 伺服器端互動回饋超時固定為 30 天（`2592000` 秒）
- 保持現有回饋流程不變，同時避免「瞬間超時」

---

## [v3.0.2] - 2026-02-11 - 桌面版 Pro 標題與視窗常駐優化

### 🌟 版本亮點
此補丁重點優化桌面版體驗：提交回饋後不再自動關閉視窗，並明確提示目前等待狀態；同時統一桌面應用標題為 Pro 命名。

### ✨ 改進功能
- 桌面應用產品名/視窗標題統一為 `MCP Feedback Enhanced Pro`
- 提交回饋後保持桌面視窗開啟，不再自動關閉
- 優化提交後提示文案，明確「等待 MCP/LLM 回應，視窗需手動關閉」

### 🐛 問題修復
- 增加舊版桌面二進制檔名相容回退（`mcp-feedback-enhanced-desktop*`）

---

## [v3.0.1] - 2026-02-11 - Pro 命名統一與 uvx 預發佈參數說明

### 🌟 版本亮點
此為小版本修正，重點在統一產品命名與完善安裝指引。CLI 輸出現已統一為 Pro 名稱，文檔與範例已補上目前 FastMCP 依賴情境所需的 uvx 預發佈參數。

### ✨ 改進功能
- CLI 版本/說明輸出統一為 `MCP Feedback Enhanced Pro`
- 文檔、範例與工作流程命令補充為 `uvx --prerelease=allow mcp-feedback-enhanced-pro ...`
- 在 README 與發佈說明中補充對上游專案 `Minidoracat/mcp-feedback-enhanced` 的致謝

### 📌 相容性說明
- 目前依賴為 `fastmcp>=3.0.0b2`，在 FastMCP 穩定版依賴落地前，uvx 安裝需帶 `--prerelease=allow`

---

## [v3.0.0] - 2026-02-11 - FastMCP 3 升級、套件名稱統一與發佈流程更新

### 🌟 版本亮點
此重大版本將專案識別統一為 `mcp-feedback-enhanced-pro`，執行基線升級至 FastMCP 3.x，並加固超時與會話保留行為，同時更新發佈流程以提供更穩定的 `uvx` 使用體驗。

### 💥 破壞性變更
- **套件與命令命名統一**：發佈套件與 CLI 入口統一為 `mcp-feedback-enhanced-pro`
- **FastMCP 基線升級**：最低執行需求升級至 FastMCP 3.x（`fastmcp>=3.0.0b2`）

### ✨ 新功能
- **會話歷史預設永久保留**：預設保留值改為 `0`（不進行過期清理）
- **支援手動刪除單筆會話歷史**：Web UI 會話管理新增單筆刪除能力

### 🚀 改進功能
- **超時機制加固**：強化超時參數處理與邊界鉗制邏輯，避免極端值導致異常
- **預設超時統一為 30 天**：後端、前端、文檔與範例統一為 `2592000` 秒
- **倉庫與維護者資訊更新**：專案連結與維護者中繼資料更新為 LeonBuild

### 🐛 問題修復
- 修復 `cleanup_temp_files(max_age=0)` 在極短時間窗口下可能遺漏新建暫存檔案的問題

### 🛠️ 發佈與工具鏈
- **致謝**：感謝 Minidoracat 與上游專案 `Minidoracat/mcp-feedback-enhanced` 提供原始架構與長期維護。
- 發佈流程統一為 `uv build --no-sources` + `uv publish`
- 新增 Trusted Publishing 相容流程，並保留 token 回退方案
