...
When the AI model calls the `get_feedback` tool, the system will automatically start the Web UI.
...
**Do NOT** manually execute commands like `uvx --prerelease=allow mcp-feedback-enhanced-pro test --web`, as this cannot integrate with the MCP system.
