...
```bash
# 安裝
pip install mcp-feedback-enhanced-pro

# 運行
mcp-feedback-enhanced-pro web
```
...
```bash
# 在遠程服務器運行
mcp-feedback-enhanced-pro web --host 0.0.0.0 --port 8000
```
...
RUN pip install mcp-feedback-enhanced-pro

EXPOSE 8000

CMD ["mcp-feedback-enhanced-pro", "web", "--host", "0.0.0.0", "--port", "8000"]
```

```bash
# 構建和運行
docker build -t mcp-feedback-enhanced-pro .
docker run -p 8000:8000 mcp-feedback-enhanced-pro
```
...
```bash
mcp-feedback-enhanced-pro web [OPTIONS]
```
...
# 解決方案：使用不同埠
mcp-feedback-enhanced-pro web --port 8001
```

2. **瀏覽器無法開啟**
```bash
# 手動開啟瀏覽器
mcp-feedback-enhanced-pro web --no-browser
...
```bash
# 啟用詳細日誌
mcp-feedback-enhanced-pro web --debug
...
```bash
# 停止服務
pkill -f mcp-feedback-enhanced-pro

# 安裝舊版本
pip install mcp-feedback-enhanced-pro==2.4.2
