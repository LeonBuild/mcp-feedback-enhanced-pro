...
## [v3.0.4] - 2026-02-11 - 恢复桌面自动关闭与强化 MCP 循环规则

### 🌟 版本亮点
此补丁恢复桌面模式提交反馈后的自动关闭行为，并强化工具定义规则，要求代理在各阶段持续循环调用交互反馈。

### ✨ 改进功能
- 恢复桌面模式：反馈提交后立即关闭桌面窗口
- 更新 `interactive_feedback` 工具规则，强化“任意阶段都需继续再次调用”
- 同步修正文案，确保提交后提示与自动关闭行为一致

---

## [v3.0.3] - 2026-02-11 - 超时安全加固

### 🌟 版本亮点
此补丁聚焦于消除交互反馈流程中的 timeout 误触发中断问题。

### ✨ 改进功能
- 从 `interactive_feedback` 工具参数中移除 `timeout`，调用方无法再传入极小值
- 服务端交互反馈超时固定为 30 天（`2592000` 秒）
- 保持现有反馈流程不变，同时避免“瞬间超时”

---

## [v3.0.2] - 2026-02-11 - 桌面版 Pro 标题与窗口常驻优化

### 🌟 版本亮点
此补丁重点优化桌面版体验：提交反馈后不再自动关闭窗口，并明确提示当前等待状态；同时统一桌面应用标题为 Pro 命名。

### ✨ 改进功能
- 桌面应用产品名/窗口标题统一为 `MCP Feedback Enhanced Pro`
- 提交反馈后保持桌面窗口开启，不再自动关闭
- 优化提交后提示文案，明确“等待 MCP/LLM 响应，窗口需手动关闭”

### 🐛 问题修复
- 增加旧版桌面二进制文件名兼容回退（`mcp-feedback-enhanced-desktop*`）

---

## [v3.0.1] - 2026-02-11 - Pro 命名统一与 uvx 预发布参数说明

### 🌟 版本亮点
这是一个小版本修复，重点统一产品命名并完善安装说明。CLI 输出现在统一为 Pro 名称，文档与示例已补充当前 FastMCP 依赖场景下所需的 uvx 预发布参数。

### ✨ 改进功能
- CLI 版本/帮助输出统一为 `MCP Feedback Enhanced Pro`
- 文档、示例与工作流命令补充为 `uvx --prerelease=allow mcp-feedback-enhanced-pro ...`
- 在 README 与发布说明中补充对上游项目 `Minidoracat/mcp-feedback-enhanced` 的致谢

### 📌 兼容性说明
- 当前依赖为 `fastmcp>=3.0.0b2`，在 FastMCP 稳定版依赖落地前，uvx 安装需带 `--prerelease=allow`

---

## [v3.0.0] - 2026-02-11 - FastMCP 3 升级、包名统一与发布流程更新

### 🌟 版本亮点
这个大版本统一了项目身份为 `mcp-feedback-enhanced-pro`，将运行时基线升级到 FastMCP 3.x，并对超时与会话保留行为进行加固，同时更新发布流程以支持更稳定的 `uvx` 使用体验。

### 💥 破坏性变更
- **包名与命令规范统一**：发布包与 CLI 入口统一为 `mcp-feedback-enhanced-pro`
- **FastMCP 基线升级**：最低运行时升级到 FastMCP 3.x（`fastmcp>=3.0.0b2`）

### ✨ 新功能
- **会话历史默认永久保留**：默认保留值改为 `0`（不做过期清理）
- **支持手动删除单条会话历史**：Web UI 会话管理新增单条删除能力

### 🚀 改进功能
- **超时机制加固**：强化超时参数处理与边界钳制逻辑，避免极端值导致异常
- **默认超时统一为 30 天**：后端、前端、文档与示例统一为 `2592000` 秒
- **仓库与维护者信息更新**：项目链接与维护者元数据更新为 LeonBuild

### 🐛 问题修复
- 修复 `cleanup_temp_files(max_age=0)` 在极短时间窗口下可能遗漏新建临时文件的问题

### 🛠️ 发布与工具链
- **致谢**：感谢 Minidoracat 与上游项目 `Minidoracat/mcp-feedback-enhanced` 提供原始架构与长期维护。
- 发布流程统一为 `uv build --no-sources` + `uv publish`
- 增加 Trusted Publishing 兼容流程，并保留 token 回退方案
...
**完整项目信息：** [GitHub - mcp-feedback-enhanced-pro](https://github.com/LeonBuild/mcp-feedback-enhanced-pro)
